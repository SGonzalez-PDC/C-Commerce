...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            SELECT @emp_cart = empresa FROM FFA..ffa_tbl_txn_header_cart WHERE cart_id = @cart_id;

            IF NOT EXISTS (
                SELECT 1 FROM FFA..ARTICULO
            ...
            FROM FFA..ffa_tbl_txn_detail_cart
            ...
            BEGIN
                ...
                INSERT INTO FFA..ffa_tbl_txn_detail_cart (
        ...
        BEGIN
            ...
            BEGIN
                SELECT * FROM FFA..ffa_tbl_txn_detail_cart 
        ...
        BEGIN
            ...
            IF @line_id IS NOT NULL AND @cart_id IS NOT NULL
            BEGIN
                DELETE FROM FFA..ffa_tbl_txn_detail_cart WHERE cart_id = @cart_id AND line_id = @line_id;
            END
            ...
            BEGIN
                ...
                SELECT line_id FROM FFA..ffa_tbl_txn_detail_cart
                WHERE cart_id = @cart_id AND sku = @sku;

                INSERT INTO @to_del (line_id)
                SELECT d.line_id FROM FFA..ffa_tbl_txn_detail_cart d
                WHERE d.cart_id = @cart_id
                  AND d.bonus_origin_line IN (SELECT line_id FROM @to_del)
                  AND d.line_id NOT IN (SELECT line_id FROM @to_del);

                DELETE FROM FFA..ffa_tbl_txn_detail_cart
