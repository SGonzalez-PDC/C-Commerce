...
BEGIN
    ...
    BEGIN TRY
        ...
        DECLARE @territorio_cliente VARCHAR(100) =
            (SELECT territorio FROM STAGE_FFA..clientes WHERE empresa = @empresa AND codcliente = @codcliente);

        ;WITH JerarquiaSegmentacion AS (
            SELECT n.id_nivel, n.empresa, n.id_nivel_padre, 0 AS prof
            FROM STAGE_FFA..FFAniveles n INNER JOIN STAGE_FFA..CLIENTES c ON c.segmentacion_cliente = n.id_nivel
            ...
            FROM STAGE_FFA..FFAniveles n INNER JOIN JerarquiaSegmentacion j ON n.id_nivel = j.id_nivel_padre
            WHERE n.status = 1 AND j.prof < 10
        ),
        JerarquiaGeografia AS (
            SELECT n.id_nivel, n.empresa, n.id_nivel_padre, 0 AS prof
            FROM STAGE_FFA..FFAniveles n INNER JOIN STAGE_FFA..CLIENTES c ON c.geografia = n.id_nivel
            WHERE c.codcliente = @codcliente AND c.ACTIVO = 'S' AND c.empresa = @empresa AND n.status = 1
            UNION ALL
            SELECT n.id_nivel, n.empresa, n.id_nivel_padre, j.prof + 1
            FROM STAGE_FFA..FFAniveles n INNER JOIN JerarquiaGeografia j ON n.id_nivel = j.id_nivel_padre
            WHERE n.status = 1 AND j.prof < 10
        )
        SELECT DISTINCT f.codigo
        INTO #promos_ok
        FROM STAGE_FFA..ffa_promocion f
        INNER JOIN STAGE_FFA..ffa_asignacion_promocion fa ON fa.id_referencia = f.codigo AND fa.empresa = f.empresa
        LEFT JOIN STAGE_FFA..ffa_asignacion_promocion_lista_detalle fad_geo ON fad_geo.id_asignacion = fa.id_asignacion AND fad_geo.empresa = fa.empresa
        LEFT JOIN STAGE_FFA..ffa_asignacion_promocion_lista_detalle fad_seg ON fad_seg.id_asignacion = fa.id_asignacion AND fad_seg.empresa = fa.empresa AND fad_seg.tipo_estructura = 13
        ...
        INTO #chosen
        FROM STAGE_FFA..c_commerce_cart_chosen_promo cp
        ...
        DELETE FROM STAGE_FFA..ffa_tbl_txn_detail_cart WHERE cart_id = @cart_id AND line_type = 'BONUS';
        ...
        FROM (SELECT d.line_id, d.sku, d.qty, d.line_subtotal FROM STAGE_FFA..ffa_tbl_txn_detail_cart d WHERE d.cart_id = @cart_id AND d.line_type = 'SALE') s
        LEFT JOIN #chosen ch ON ch.sku = s.sku
        CROSS APPLY (
            SELECT TOP 1 p.codigo, p.nombre, bp.porcentaje_descuento AS pct
            FROM STAGE_FFA..ffa_promocion_articulo pa
            INNER JOIN STAGE_FFA..ffa_promocion p ON p.empresa = pa.empresa AND p.codigo = pa.codigo_promocion
            INNER JOIN STAGE_FFA..ffa_beneficios_promocion bp ON bp.empresa = p.empresa AND bp.codigo_promocion = p.codigo AND bp.tipo_beneficio_id = 1 AND bp.porcentaje_descuento > 0
            INNER JOIN STAGE_FFA..ffa_condiciones_promocion c ON c.empresa = p.empresa AND c.codigo_promocion = p.codigo
        ...
        FROM STAGE_FFA..ffa_tbl_txn_detail_cart d INNER JOIN @aplicar a ON a.line_id = d.line_id WHERE d.cart_id = @cart_id;

        INSERT INTO STAGE_FFA..ffa_tbl_txn_discount_cart (cart_id, line_id, discount_seq, tipo_forma, cod_promocion, tipo_valor, valor, descuento_calc, id_descuento, observaciones, estado)
        ...
        DECLARE @maxline INT = ISNULL((SELECT MAX(line_id) FROM STAGE_FFA..ffa_tbl_txn_detail_cart WHERE cart_id = @cart_id), 0);
        ;WITH bonus_all AS (
            ...
            INNER JOIN STAGE_FFA..ffa_promocion_articulo pa ON pa.empresa = @empresa AND pa.articulo_sku = d.sku
            INNER JOIN STAGE_FFA..ffa_promocion p ON p.empresa = pa.empresa AND p.codigo = pa.codigo_promocion
            INNER JOIN STAGE_FFA..ffa_beneficios_promocion bp ON bp.empresa = p.empresa AND bp.codigo_promocion = p.codigo AND bp.tipo_beneficio_id = 2 AND bp.cantidad_redimible > 0
            OUTER APPLY (
        ...
        INSERT INTO STAGE_FFA..ffa_tbl_txn_detail_cart (cart_id, line_id, sku, line_type, qty, unit_price, line_subtotal, line_discounts, line_total, bonus_origin_line, created_at, updated_at)
        ...
        CROSS APPLY (
            ...
            FROM STAGE_FFA..ffa_tbl_txn_detail_cart d WHERE d.cart_id = @cart_id
        ) x
        WHERE h.cart_id = @cart_id;
