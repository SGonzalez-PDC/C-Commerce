...
BEGIN
    ...
    BEGIN TRY
        ...
        ;WITH JerarquiaSegmentacion AS (
            ...
            FROM PREP_FFA..FFAniveles n
            INNER JOIN PREP_FFA..CLIENTES c ON c.segmentacion_cliente = n.id_nivel
        ...
        JerarquiaGeografia AS (
            SELECT n.id_nivel, n.empresa, n.id_nivel_padre, 0 AS prof
            FROM PREP_FFA..FFAniveles n
            INNER JOIN PREP_FFA..CLIENTES c ON c.geografia = n.id_nivel
            WHERE c.codcliente = @codcliente AND c.ACTIVO = 'S' AND c.empresa = @empresa AND n.status = 1
            UNION ALL
            SELECT n.id_nivel, n.empresa, n.id_nivel_padre, j.prof + 1
            FROM PREP_FFA..FFAniveles n
        ...
        INNER JOIN PREP_FFA..ffa_asignacion_promocion fa
            ON fa.id_referencia = f.codigo AND fa.empresa = f.empresa
        LEFT JOIN PREP_FFA..ffa_asignacion_promocion_lista_detalle fad_geo
            ON fad_geo.id_asignacion = fa.id_asignacion AND fad_geo.empresa = fa.empresa
        LEFT JOIN PREP_FFA..ffa_asignacion_promocion_lista_detalle fad_seg
        ...
        FROM #lineas l
        INNER JOIN PREP_FFA..ffa_promocion_articulo pa
            ON pa.empresa = @empresa AND pa.articulo_sku = l.sku
        INNER JOIN PREP_FFA..ffa_promocion p
        ...
        INNER JOIN PREP_FFA..ffa_beneficios_promocion bp
        ...
        LEFT JOIN PREP_FFA..ARTICULO ab
            ON ab.EMPRESA = @empresa AND ab.SKU = bp.articulo_sku
        OUTER APPLY (
            SELECT TOP 1 ri.LINK FROM PREP_FFA..RED_IMAGENES ri
